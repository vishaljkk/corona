...
for i in range(int(int(len(finalgoldrate))/2),len(finalgoldrate)):
    today, yesterday = finalgoldrate[i], finalgoldrate[i-1]
    gold_deltas.append(((today - yesterday) / today) * 100)
print("finally thre new values are")
print(new_xvalues)
print(gold_deltas)
print(len(new_xvalues), len(growth_rate), len(gold_deltas))
...
plt.scatter(new_xvalues, gold_deltas)
plt.plot(new_xvalues, gold_deltas,label = "line 2")
plt.xlabel('x - axis')
